...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        } else Bukkit.getLogger().log(Level.WARNING, ChatColor.DARK_RED + "[Tournaments] Вы должны быть игроком, чтобы использовать эту команду!");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String aliases, String[] args) {
        if (command.getName().equalsIgnoreCase("tournaments")) {
            if (args.length == 1) return commands.stream().map(cmd -> cmd.getSimpleName().toLowerCase()).collect(Collectors.toList());
        }
        return null;
    }
}
